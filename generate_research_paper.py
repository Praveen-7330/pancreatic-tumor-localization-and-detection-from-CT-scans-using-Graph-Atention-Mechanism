...
def create_research_paper():
    paper_md = """# Pancreatic Tumor Localization and Detection from CT Scans Using Graph Attention Mechanisms
...
3. **Intensity Normalization:** Z-score normalization scales voxel intensities to zero mean and unit variance:
   $$I_{\text{norm}} = \frac{I_{\text{win}} - \mu}{\sigma}$$
...
The encoder consists of 4 sequential downsampling blocks. Each block comprises two $3 \times 3 \times 3$ 3D convolutions, Instance Normalization, and LeakyReLU activation ($\alpha=0.01$), followed by a $2 \times 2 \times 2$ max-pooling operation. Feature channel dimensions expand sequentially across levels: $C \in \{16, 32, 64, 128\}$. For an input volume patch $X \in \mathbb{R}^{1 \times 96 \times 96 \times 96}$, the bottleneck outputs a feature map $F_b \in \mathbb{R}^{128 \times 12 \times 12 \times 12}$.
...
The constructed graph $\mathcal{G} = (\mathcal{V}, \mathcal{E})$ is passed through a multi-head GATv2 layer. GATv2 computes dynamic attention coefficients $\alpha_{ij}$ measuring the relative importance of node $j$ to node $i$:
...
$$\mathbf{h}'_i = \|_{m=1}^M \sigma \left( \sum_{j \in \mathcal{N}_i} \alpha_{ij}^m \mathbf{W}^m \mathbf{h}_j \right)$$
...
$$\mathcal{L}_{\text{Dice}} = 1 - \frac{2 \sum_{c=1}^2 \sum_v p_{c,v} g_{c,v} + \epsilon}{\sum_{c=1}^2 \sum_v (p_{c,v}^2 + g_{c,v}^2) + \epsilon}$$

$$\mathcal{L}_{\text{Focal}} = -\frac{1}{V} \sum_{v=1}^V \sum_{c=1}^2 (1 - p_{c,v})^\gamma \log(p_{c,v})$$
...
"""
    ...
    # Save .html (Styled Academic Paper Format)
    html_path = "RESEARCH_PAPER.html"
    html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>Pancreatic Tumor Localization & Segmentation Research Paper</title>
    <style>
        body {{
        ...
        h3 {{ font-size: 12pt; margin-top: 20px; color: #1e293b; }}
        table {{
...
<body>
"""
    # Simple line-by-line converter for HTML formatting
    ...
    for line in lines:
        ...
        if line.startswith("|") and "|" in line[1:]:
            ...
            if "---" in parts[0]:
                continue
            is_header = "Architecture" in line or "Target Class" in line or "Feature Component" in line
        ...
        if line.startswith("# "):
            ...
            html_content += f"<div class='abstract-box'><strong>ABSTRACT:</strong> "
        elif line.startswith("## "):
            html_content += f"<h2>{line[3:]}</h2>\n"
        elif line.startswith("### "):
            html_content += f"<h3>{line[4:]}</h3>\n"
        elif line.startswith("**Keywords:**"):
            html_content += f"<p><strong>Keywords:</strong> {line[13:]}</p></div>\n"
        elif line.strip() == "---":
            html_content += "<hr>\n"
        elif line.strip() == "":
            html_content += "<br>\n"
